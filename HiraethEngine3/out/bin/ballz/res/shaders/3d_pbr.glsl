...
void main(void) {
	...
	vec3 ambientLightDirection = vec3(0);
	...
	for(int i = 0; i < numLights; ++i) { 
		...
		if(i == 0)
			ambientLightDirection = lightDirection.xyz;
		
		vec3 halfwayDirection = normalize(lightDirection.xyz + viewDirection);
		shadow = calculateShadows(pass_normal, ambientLightDirection);
	...
	vec3 ambient = (kD * diffuse + specular) * ao * shadow;

	out_colour = vec4(ambient + totalLight + u_emission.rgb, 1.0);
	//out_colour = vec4(vec3(shadow), 1.0);
	//out_colour = vec4(pass_shadowCoords.xy, 0.0, 1.0);
	//out_colour = vec4(normalRgb, 1.0);
}