...
extern void hnConnectClient(HnClient* client, const std::string& host, const unsigned int port);
// disconnects and cleans up given client
extern void hnDisconnectClient(HnClient* client);
...
extern void hnHandleClientPacket(HnClient* client, const HnPacket& packet);
// requests a new variable which will be registered anywhere. syncRate is the number of ticks after which the variable 
// should be synced with 
extern void hnCreateVariable(HnClient* client, const std::string& name, const HnDataType type, const unsigned int syncRate);
...
extern void hnSyncClient(HnClient* client);
...
extern inline void hnHookVariable(HnClient* client, const std::string& name, void* data);
...
extern inline void hnHookVariable(HnClient* client, HnLocalClient* localclient, const std::string& name, void* data);
// waits for the next packet read by the client, parses the information and returns the packet
extern HnPacket hnReadClientPacket(HnClient* client);
...
extern HnPacket hnGetCustomPacket(HnLocalClient* client);
